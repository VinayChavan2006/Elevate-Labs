...
import { deleteChatMessages } from './messageController.js'
...
export const createGroupChat = async(req, res) => {
    try {
        ...
        // check if this chat with same name exists
        const chat = await Chat.findOne({
            chatName
        })
        ...
        if(chat){
            return res.status(400).json({
                success:false,
                message: "Chat with this name exists"
            })
        } 
        ...
        const users = await User.find({_id: {$in: [...members, req.user?._id]}})
